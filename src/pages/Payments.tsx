...
const Payments: React.FC = () => {
  ...
  const handleAddPayment = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await createDocument('payments', newPayment);
            ...
            <form onSubmit={handleAddPayment} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="amount" required type="number" placeholder="0.00" className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Payment Date</label>
                  <input name="date" required type="date" defaultValue={format(new Date(), 'yyyy-MM-dd')} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">Payment Mode</label>
                  <select name="mode" required className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                ...
                <div>
                  ...
                  <select name="bankId" className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
              ...
              <div>
                ...
                <textarea name="notes" rows={2} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" placeholder="Transaction ID, etc."></textarea>
              </div>
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" className="flex-1 px-4 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100">Record Payment</button>
              </div>
            </form>
          </div>
        </div>
